...
class GUI(UI):
    ...
    def __init__(self):
        ...
        self.tasks = Subject()
        self.threads = [
            threading.Thread(target=self.data_binding)
        ]
    ...
    def select_storage(self):
        ...
        # Labels
        title = tk.Label(
            storage_choice_window, text="Wilkommen bei der Dank Bank", font=("Calibri", 14)
        )
...
class Toolbar(tk.Frame):
    def __init__(self, root, state: GUI, *args, **kwargs) -> None:
        ...
        self.user_logout_button = tk.Button(
            self.user_actions, text="Logout"
        )
        self.user_logout_button.pack(side=tk.LEFT)
        self.user_label = tk.Label(
            self.user_actions, text="Eingeloggt als ?"
        )
        self.user_label.pack(side=tk.RIGHT)

        # Konto Actions
        self.konto_actions = tk.Frame(self)
        self.konto_logout_button = tk.Button(
            self.konto_actions, text="Konto wechseln"
        )
        self.konto_logout_button.pack(side=tk.LEFT)
        self.konto_label = tk.Label(
            self.konto_actions, text="Konto ?"
        )
        ...
        self.back_button = tk.Button(
            self,
            text="Zurück",
            command=self.state.history.backward_switch_menu
        )
...
class UserMenu:
    ...
    def main_menu(self):
        ...
        for text, action in actions:
            tk.Button(
                self.view, text=text, command=action
            ).pack()
    ...
    def login_action(self, username: str, password: str) -> None:
        ...
        if not user:
            self.master.window.error = "Bitch"
        else:
            self.user = user
    ...
    def delete_action(self, username: str, password: str) -> None:
        ...
        if not user:
            self.error_field.config(text="Bitch")
        else:
            user.objects.delete()
